...
<div class="pt-[130px] md:pt-[250px] pb-24 max-w-[1200px] mx-auto px-4 md:px-6 min-h-screen">
  ...
  <div x-data="ccCalendarPage()" x-init="init()">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
      <h1 class="text-3xl md:text-4xl font-extrabold text-black tracking-tight">All Shows</h1>
          ...
          <div class="p-5 flex flex-col flex-1">
            ...
            <div class="mt-auto">
              <template x-if="show.status !== 'Sold Out'">
                <a :href="'?view=event&show=' + show.id" class="inline-block px-6 py-2.5 bg-black text-white font-bold text-sm rounded-[10px] hover:bg-neutral-800 transition-colors">Buy Tickets</a>
              </template>
              <template x-if="show.status === 'Sold Out'">
                <button disabled class="px-6 py-2.5 bg-neutral-200 text-neutral-400 font-bold text-sm rounded-[10px] cursor-not-allowed">Sold Out</button>
              </template>
            </div>
          </div>
        </div>
    ...
    <div :class="viewMode === 'list' ? '' : 'md:hidden'" class="space-y-4">
      <template x-for="show in visibleShows" :key="show.id">
        <div class="bg-white rounded-xl border border-neutral-200 shadow-md p-5 flex flex-col md:flex-row md:items-center gap-5">
          ...
          <template x-if="show.status !== 'Sold Out'">
            <a :href="'?view=event&show=' + show.id" class="w-full md:w-auto text-center px-6 py-2.5 bg-black text-white font-bold text-sm rounded-[10px] hover:bg-neutral-800 transition-colors whitespace-nowrap flex-shrink-0">Buy Tickets</a>
          </template>
          <template x-if="show.status === 'Sold Out'">
            <button disabled class="w-full md:w-auto text-center px-6 py-2.5 bg-neutral-200 text-neutral-400 font-bold text-sm rounded-[10px] cursor-not-allowed whitespace-nowrap flex-shrink-0">Sold Out</button>
          </template>
        </div>
      </template>
    </div>
