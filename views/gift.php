...
<div class="pt-[150px] pb-24 container mx-auto px-6" x-data="{ amount: 50 }">
  <!-- Header -->
        ...
        <div class="relative h-full p-8 flex flex-col justify-between">
          <div class="flex justify-between items-start">
            <div>
              ...
              <p class="text-[#24CECE] text-sm font-semibold tracking-widest mt-1">COMEDY CLUB</p>
            </div>
      ...
      <form method="POST" action="?view=gift" class="bg-white pt-0 pb-8 px-8 md:px-10 rounded-[5px] border border-neutral-200 shadow-xl space-y-8 text-neutral-900">
        ...
        <div>
          ...
          <div class="grid grid-cols-4 gap-4 mb-4">
            <button type="button" @click="amount = 25" :class="amount === 25 ? 'bg-[#24CECE] text-black border-[#24CECE]' : 'bg-neutral-50 text-neutral-600 border-neutral-200 hover:border-[#24CECE]/50'" class="cc-amount-btn py-3 rounded-[5px] font-bold border transition-all">$25</button>
            <button type="button" @click="amount = 50" :class="amount === 50 ? 'bg-[#24CECE] text-black border-[#24CECE]' : 'bg-neutral-50 text-neutral-600 border-neutral-200 hover:border-[#24CECE]/50'" class="cc-amount-btn py-3 rounded-[5px] font-bold border transition-all">$50</button>
            <button type="button" @click="amount = 75" :class="amount === 75 ? 'bg-[#24CECE] text-black border-[#24CECE]' : 'bg-neutral-50 text-neutral-600 border-neutral-200 hover:border-[#24CECE]/50'" class="cc-amount-btn py-3 rounded-[5px] font-bold border transition-all">$75</button>
            <button type="button" @click="amount = 100" :class="amount === 100 ? 'bg-[#24CECE] text-black border-[#24CECE]' : 'bg-neutral-50 text-neutral-600 border-neutral-200 hover:border-[#24CECE]/50'" class="cc-amount-btn py-3 rounded-[5px] font-bold border transition-all">$100</button>
          </div>
          <div class="relative">
            <span class="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-500">$</span>
            <input type="number" name="amount" x-model.number="amount" min="1" placeholder="Custom Amount" class="w-full bg-neutral-50 border border-neutral-200 rounded-[5px] py-3 pl-8 pr-4 text-neutral-900 focus:ring-2 focus:ring-[#24CECE] focus:border-transparent outline-none placeholder:text-neutral-400" required />
          </div>
        </div>
