        ...
        <div class="relative w-full md:w-auto">
          ...
          <div x-show="calendarOpen" x-transition @click.outside="calendarOpen = false"
            ...
            <div class="grid grid-cols-7 gap-1">
              <template x-for="n in calendarOffset" :key="'blank-'+n"><div></div></template>
...
<script>
function ccPromoPage() {
  return {
    allShows: <?= json_encode(array_values($promoShows)) ?>,
    ...
    hasShow(d) { return this.showDates.indexOf(this.dateStr(d)) !== -1; },

    get calendarOffset() { return new Date(this.calYear, this.calMonth, 1).getDay(); },
    get calendarDays() {
      const daysInMonth = new Date(this.calYear, this.calMonth + 1, 0).getDate();
      const today = new Date(); const todayStr = this.dateStr(today); const cells = [];
      for (let day = 1; day <= daysInMonth; day++) {
        const d = new Date(this.calYear, this.calMonth, day); const ds = this.dateStr(d);
        cells.push({ key: ds, day, isSelected: this.activeDate === ds, isToday: ds === todayStr, hasShows: this.hasShow(d) });
      }
      return cells;
    },
    get filteredShows() { return this.activeDate ? this.allShows.filter(s => s.date.startsWith(this.activeDate)) : this.allShows; },
    get visibleShows() { return this.filteredShows.slice(0, this.showLimit); },
    get hasMore() { return this.filteredShows.length > this.showLimit; },
    showMore() { this.showLimit = this.filteredShows.length; },

    formatPill(isoDate, part) {
      const d = new Date(isoDate);
      const days = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
      const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
      if (part === 'date') return days[d.getDay()] + ', ' + d.getDate() + ' ' + months[d.getMonth()] + ' ' + d.getFullYear();
      if (part === 'time') { let h = d.getHours(), m = d.getMinutes(), ap = h >= 12 ? 'PM' : 'AM'; h = h % 12 || 12; return (m === 0 ? h + ':00' : h + ':' + this.pad(m)) + ap; }
    },
    formatDateLabel(ds) {
    ...
    onCalendarDayClick(cell) {
      if (this.activeDate === cell.key) { this.activeDate = null; this.showLimit = 12; }
      else { this.activeDate = cell.key; this.showLimit = 100; }
      this.calendarOpen = false;
    },
    clearFilter() { this.activeDate = null; this.showLimit = 12; this.calendarOpen = false; },
    calPrevMonth() { let m = this.calMonth - 1, y = this.calYear; if (m < 0) { m = 11; y--; } this.calMonth = m; this.calYear = y; },
    calNextMonth() { let m = this.calMonth + 1, y = this.calYear; if (m > 11) { m = 0; y++; } this.calMonth = m; this.calYear = y; }
  }
}
</script>
