...
<div class="pt-10 pb-16 md:pt-12 md:pb-24 max-w-[1400px] mx-auto px-4 md:px-6 min-h-screen"
  ...
  <div class="relative mb-8 md:mb-16">
    <span aria-hidden="true" class="hidden md:block absolute -top-10 -left-4 text-[220px] lg:text-[260px] font-black text-black/[0.04] select-none leading-none pointer-events-none tracking-tight">02</span>
    ...
    <p class="relative mt-6 text-neutral-600 text-base max-w-lg">
  ...
  <div class="grid md:grid-cols-12 gap-6 md:gap-12 items-start">
    ...
    <div class="md:col-span-7">
      <div class="grid sm:grid-cols-2 gap-4">
        <?php foreach ($addonCatalog as $addon):
          $k = $addon['key'];
        ?>
        <div class="group relative bg-neutral-50 border border-black/10 hover:border-[#d12027]/40 rounded-xl p-5 transition-colors"
             :class="addons.<?= $k ?> > 0 ? 'ring-2 ring-[#d12027]/40 border-[#d12027]/40' : ''">
          <div class="flex items-start gap-4 mb-4">
            <span class="shrink-0 w-11 h-11 flex items-center justify-center rounded-lg bg-[#d12027]/10 border border-[#d12027]/20 text-[#d12027]">
              <i data-lucide="<?= $addon['icon'] ?>" class="w-5 h-5"></i>
            </span>
            <div class="flex-1 min-w-0">
              <div class="flex items-baseline gap-2 mb-1">
                <h3 class="text-base font-extrabold text-neutral-900"><?= htmlspecialchars($addon['label']) ?></h3>
                <span class="text-[#d12027] font-extrabold text-sm">+$<?= number_format($addon['price'], 0) ?></span>
              </div>
              <p class="text-xs text-neutral-600 leading-snug"><?= $addon['desc'] ?></p>
            </div>
          </div>
          <div class="flex items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-1 bg-black/5 border border-black/10 rounded-[10px] p-1">
              <button type="button" @click="addons.<?= $k ?> = Math.max(0, addons.<?= $k ?> - 1)" class="w-8 h-8 flex items-center justify-center text-neutral-900 hover:bg-black/10 rounded-[6px] transition-colors" aria-label="Decrease">−</button>
              <span class="w-8 text-center text-neutral-900 font-extrabold text-sm" x-text="addons.<?= $k ?>"></span>
              <button type="button" @click="addons.<?= $k ?> = Math.min(10, addons.<?= $k ?> + 1)" class="w-8 h-8 flex items-center justify-center text-neutral-900 hover:bg-black/10 rounded-[6px] transition-colors" aria-label="Increase">+</button>
            </div>
          </div>
        </div>
        <?php endforeach; ?>
    ...
    <aside class="md:col-span-5">
      <div class="md:sticky md:top-44 bg-neutral-50 border border-black/10 rounded-xl overflow-hidden">

        <!-- Show summary -->
        <div class="p-5 md:p-8 border-b border-dashed border-black/15">
          ...
          <h2 class="text-xl md:text-2xl font-extrabold tracking-tight leading-[1.1] text-neutral-900 mb-1"><?= htmlspecialchars($cityState) ?></h2>
          <div class="text-base font-extrabold text-[#d12027] tracking-tight"><?= htmlspecialchars($venue) ?></div>
        </div>

        <!-- Tickets breakdown -->
        <div class="p-5 md:p-8 border-b border-black/10">
          <p class="text-[10px] font-extrabold tracking-[0.3em] uppercase text-[#d12027] mb-4">Your Tickets</p>
          <div class="space-y-2">
            <?php foreach ($tickets as $k => $qty):
              if (!isset($tierInfo[$k]) || $qty <= 0) continue;
            ?>
            <div class="flex items-center justify-between text-sm">
              <span class="text-neutral-600"><?= $qty ?> × <?= htmlspecialchars($tierInfo[$k]['label']) ?></span>
        ...
        <div class="p-6 md:p-8">
          <div class="space-y-2 mb-4">
            <div class="flex items-center justify-between text-sm">
              <span class="text-neutral-600">Tickets subtotal</span>
              <span class="text-neutral-900 font-bold">$<?= number_format($ticketTotal, 0) ?></span>
            </div>
            <div class="flex items-center justify-between text-sm">
              <span class="text-neutral-600"><span x-text="addonsQty"></span> add-on<span x-show="addonsQty !== 1">s</span></span>
              <span class="text-neutral-900 font-bold" x-text="'$' + addonsTotal.toFixed(0)"></span>
            </div>
          </div>
          <div class="h-px bg-black/10 mb-4"></div>
          <div class="flex items-end justify-between mb-6">
            <span class="text-[10px] font-extrabold tracking-[0.3em] uppercase text-[#d12027]">Running Total</span>
            <span class="text-3xl font-extrabold text-neutral-900 tracking-tight" x-text="'$' + grand.toFixed(0)"></span>
          </div>
          ...
          <a href="?view=tour&tour=ilct-nyc" class="mt-4 block text-center w-full text-sm text-neutral-600 hover:text-[#d12027] transition-colors">← Pick a different date</a>
        </div>
      </div>
    </aside>
