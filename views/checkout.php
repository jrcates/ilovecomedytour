...
<div class="pt-10 pb-16 md:pt-12 md:pb-24 max-w-[1400px] mx-auto px-4 md:px-6 min-h-screen"
  ...
  <div class="flex items-center gap-3 text-[10px] font-extrabold uppercase tracking-[0.3em] mb-8">
    ...
    <span class="text-white/20">/</span>
    <a href="?view=addons&show=<?= urlencode($showId) ?>&tickets=<?= urlencode($ticketsParam) ?>" class="text-neutral-500 hover:text-[#f9dda9] transition-colors">Add-ons</a>
    <span class="text-white/20">/</span>
    <span class="text-[#f9dda9]">Step 03 · Checkout</span>
  </div>

  <!-- ─── Header ─── -->
  <div class="relative mb-8 md:mb-16">
    <span aria-hidden="true" class="hidden md:block absolute -top-10 -left-4 text-[220px] lg:text-[260px] font-black text-white/[0.04] select-none leading-none pointer-events-none tracking-tight">03</span>
  ...
  <form action="?view=thank-you" method="get" class="grid lg:grid-cols-2 gap-6 lg:gap-8">
    ...
    <div class="space-y-8" :class="{'hidden lg:block': mobileStep !== 1}">
      ...
      <div>
        ...
        <h2 class="text-3xl md:text-5xl font-extrabold text-white tracking-tight leading-[1.05] mb-2"><?= htmlspecialchars($cityState) ?></h2>
        <div class="text-xl md:text-2xl font-extrabold text-[#f9dda9] tracking-tight mb-3"><?= htmlspecialchars($venue) ?></div>
        <?php if (!empty($show['description'])): ?>
        <p class="text-sm md:text-base text-neutral-400 leading-relaxed mb-3"><?= htmlspecialchars($show['description']) ?></p>
        <?php endif; ?>
      ...
      <div class="pt-6 border-t border-white/10">
        <p class="text-[10px] font-extrabold tracking-[0.3em] uppercase text-[#f9dda9] mb-5">Order Summary</p>
        ...
        <div class="space-y-4 mb-5">
          ...
          <div class="flex items-start justify-between gap-4">
            <div>
              <div class="text-white font-extrabold"><?= htmlspecialchars($tierInfo[$k]['label']) ?></div>
              <div class="text-xs text-neutral-500 mt-0.5"><?= $qty ?> × $<?= number_format($tierInfo[$k]['price'], 2) ?></div>
            </div>
            <div class="text-white font-extrabold">$<?= number_format($line, 2) ?></div>
          </div>
          <?php endforeach; ?>
          ...
          <div class="flex items-start justify-between gap-4">
            <div>
              <div class="text-white font-extrabold"><?= htmlspecialchars($addonInfo[$k]['label']) ?></div>
        ...
        <!-- Tax / Discounts -->
        <div class="pt-4 border-t border-white/10 space-y-2 mb-4">
          <div x-show="promoApplied" class="flex justify-between text-sm">
            <span class="text-[#f9dda9] font-semibold">Promo (10% off)</span>
            <span class="text-[#f9dda9] font-bold" x-text="'− ' + fmt(discount)"></span>
          </div>
          <div class="flex justify-between text-sm">
            <span class="text-neutral-400">Sales Tax (<?= $salesTaxPct ?>%)</span>
            <span class="text-white" x-text="fmt(tax)"></span>
          </div>
          ...
          <div x-show="giftApplied" class="flex justify-between text-sm">
            ...
            <span class="text-[#f9dda9] font-bold" x-text="'− ' + fmt(giftUse)"></span>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between pt-4 border-t border-white/10">
          ...
          <span class="text-xl font-extrabold text-white" x-text="fmt(total)"></span>
        </div>

        <!-- Promo code -->
        <div class="mt-6 pt-6 border-t border-white/10">
          <label class="flex items-center gap-3 cursor-pointer select-none">
            <input type="checkbox" x-model="showPromo" class="w-4 h-4 rounded border-white/20 bg-white/5 text-[#f9dda9] focus:ring-[#f9dda9] focus:ring-offset-0" />
            <span class="text-sm text-white font-semibold">I have a promo code</span>
            <span x-show="promoApplied" class="inline-flex items-center gap-1 text-[10px] font-extrabold tracking-[0.2em] uppercase text-[#f9dda9]">
          ...
          <div x-show="showPromo" x-transition class="mt-4 space-y-2">
            <div class="flex items-end gap-4">
              <input type="text" name="promo" x-model="promoCode" @keydown.enter.prevent="applyPromo()" placeholder="Try 1111"
                     class="flex-1 bg-transparent border-b-2 border-white/15 focus:border-[#f9dda9] py-3 text-white text-sm placeholder:text-white/25 focus:outline-none transition-colors" />
              <button type="button" @click="applyPromo()" class="shrink-0 pb-3 text-[10px] font-extrabold uppercase tracking-[0.3em] text-[#f9dda9] hover:text-white transition-colors">Apply</button>
            </div>
            <p x-show="promoMsg" x-transition :class="promoMsgKind === 'ok' ? 'text-[#f9dda9]' : 'text-[#ff6b6b]'" class="text-xs font-semibold" x-text="promoMsg"></p>
          </div>
        </div>
    ...
    <aside :class="{'hidden lg:block': mobileStep !== 2}">
      ...
      <button type="button" @click="mobileStep = 1; window.scrollTo({ top: 0, behavior: 'smooth' })"
              class="lg:hidden inline-flex items-center gap-2 text-sm font-bold text-[#f9dda9] hover:text-white transition-colors mb-5">
        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
        Back to Summary
      </button>
      <div class="lg:sticky lg:top-44 bg-[#1e1e1e] rounded-xl border border-white/10 overflow-hidden">
      <div class="p-5 md:p-8 space-y-6 md:space-y-8">

        <!-- Customer Information -->
        <div>
          <header class="flex items-center gap-3 pb-5 mb-6 border-b border-white/10">
            <span class="w-10 h-10 rounded-lg bg-[#f9dda9]/10 border border-[#f9dda9]/20 flex items-center justify-center flex-shrink-0">
              <i data-lucide="user" class="w-5 h-5 text-[#f9dda9]"></i>
            </span>
            <h2 class="text-lg font-extrabold text-white">Customer Information</h2>
          </header>
        ...
        <div>
          <header class="flex items-center gap-3 pb-5 mb-6 border-b border-white/10">
            <span class="w-10 h-10 rounded-lg bg-[#f9dda9]/10 border border-[#f9dda9]/20 flex items-center justify-center flex-shrink-0">
              <i data-lucide="credit-card" class="w-5 h-5 text-[#f9dda9]"></i>
            </span>
            <h2 class="text-lg font-extrabold text-white">Payment Information</h2>
          </header>
          ...
          <label class="flex items-center gap-3 mt-6 cursor-pointer select-none">
            <input type="checkbox" x-model="useGift" class="w-4 h-4 rounded border-white/20 bg-white/5 text-[#f9dda9] focus:ring-[#f9dda9] focus:ring-offset-0" />
            <span class="text-sm text-white font-semibold">Use Gift Certificate</span>
            <span x-show="giftApplied" class="inline-flex items-center gap-1 text-[10px] font-extrabold tracking-[0.2em] uppercase text-[#f9dda9]">
              <i data-lucide="check" class="w-3 h-3"></i> Applied
            </span>
          </label>
          <div x-show="useGift" x-transition class="mt-4 space-y-2">
            <div class="flex items-end gap-4">
              <input type="text" name="gift" x-model="giftCode" @keydown.enter.prevent="applyGift()" placeholder="Try 2222"
                     class="flex-1 bg-transparent border-b-2 border-white/15 focus:border-[#f9dda9] py-3 text-white text-sm placeholder:text-white/25 focus:outline-none transition-colors" />
              <button type="button" @click="applyGift()" class="shrink-0 pb-3 text-[10px] font-extrabold uppercase tracking-[0.3em] text-[#f9dda9] hover:text-white transition-colors">Apply</button>
            </div>
            <p x-show="giftMsg" x-transition :class="giftMsgKind === 'ok' ? 'text-[#f9dda9]' : 'text-[#ff6b6b]'" class="text-xs font-semibold" x-text="giftMsg"></p>
          </div>
        </div>

        <!-- You will be charged -->
        <div class="flex items-center justify-between px-5 py-4 border-2 border-[#f9dda9] rounded-[10px] bg-[#f9dda9]/[0.04]">
          <span class="text-[11px] font-extrabold uppercase tracking-[0.2em] text-[#f9dda9]">You will be charged</span>
          <span class="text-2xl font-extrabold text-white" x-text="fmt(total)"></span>
        </div>
        ...
        <div>
          ...
          <a href="?view=addons&show=<?= urlencode($showId) ?>&tickets=<?= urlencode($ticketsParam) ?>" class="mt-4 block text-center w-full text-sm text-neutral-400 hover:text-[#f9dda9] transition-colors">← Back to Add-ons</a>
        </div>

      </div>

      <!-- Footer strip with Secure Transaction (flush to card bottom) -->
      <div class="bg-black/30 border-t border-white/10 px-6 md:px-8 py-5">
