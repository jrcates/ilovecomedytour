...
<div x-data="{
    ...
    <div class="max-w-[1400px] mx-auto px-6 md:px-10 relative">
      ...
      <div class="md:hidden flex items-center justify-between py-5">
        ...
        <button @click="mobileOpen = true" aria-label="Open menu"
                class="w-10 h-10 flex items-center justify-center rounded-[10px] bg-white/10 border border-white/20 text-white hover:bg-white/20 transition-colors">
  ...
  <div x-cloak :class="mobileOpen ? 'translate-x-0' : 'translate-x-full'"
    ...
    <!-- Hero mascot -->
    <div class="flex justify-center px-6 pt-10 pb-6">
      <img src="assets/ilct-logo.png" alt="I Love Comedy Tour" class="h-[160px] w-auto object-contain" />
    </div>

    <!-- Editorial nav list -->
    <nav class="flex-1 px-6 pt-6 pb-6">
      <div class="flex items-center gap-3 mb-6">
        <span class="text-[10px] font-extrabold tracking-[0.3em] text-[#d12027] uppercase">Explore</span>
      ...
      ?>
      <a href="<?= htmlspecialchars($href) ?>" @click="mobileOpen = false" class="group flex items-center gap-4 py-5 border-b border-black/10 transition-colors">
        <span class="text-[10px] font-extrabold tracking-[0.24em] text-[#d12027]/50 uppercase w-12">N°<?= $num ?></span>
        <span class="flex-1 text-3xl font-extrabold tracking-tight <?= $isActive ? 'text-[#d12027]' : 'text-neutral-900 group-hover:text-[#d12027]' ?> transition-colors"><?= htmlspecialchars($ml['label']) ?></span>
        <svg class="w-5 h-5 <?= $isActive ? 'text-[#d12027]' : 'text-neutral-400 group-hover:text-[#d12027] group-hover:translate-x-1' ?> transition-all" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
      </a>
      <?php endforeach; ?>
    </nav>

    <!-- Bottom panel: sticker + follow + copyright -->
    <div class="px-6 pt-8 pb-10 bg-neutral-50 border-t border-black/10">

      <!-- Rotated red sticker -->
      <div class="flex justify-start mb-6">
        <div class="cc-sticker-sm gap-2 bg-[#d12027] text-white text-[10px] font-extrabold uppercase tracking-[0.22em] px-3 py-1.5 -rotate-2">
      ...
      <!-- Follow -->
      <p class="text-[10px] font-extrabold tracking-[0.3em] text-[#d12027] uppercase mb-4">Follow Along</p>
      <div class="flex flex-wrap gap-2 mb-8">
        <a href="#" aria-label="Facebook" class="w-11 h-11 flex items-center justify-center bg-black/5 hover:bg-[#f9dda9] hover:text-black text-neutral-700 border border-black/10 transition-colors">
          <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"/></svg>
        </a>
        <a href="#" aria-label="Instagram" class="w-11 h-11 flex items-center justify-center bg-black/5 hover:bg-[#f9dda9] hover:text-black text-neutral-700 border border-black/10 transition-colors">
        ...
        <a href="#" aria-label="X" class="w-11 h-11 flex items-center justify-center bg-black/5 hover:bg-[#f9dda9] hover:text-black text-neutral-700 border border-black/10 transition-colors">
          <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
        </a>
        <a href="#" aria-label="YouTube" class="w-11 h-11 flex items-center justify-center bg-black/5 hover:bg-[#f9dda9] hover:text-black text-neutral-700 border border-black/10 transition-colors">
