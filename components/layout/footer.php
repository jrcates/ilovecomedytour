<!-- ─── Footer ─── -->
<footer class="bg-neutral-900 pt-16 pb-8 border-t border-white/10">
  <div class="max-w-[1200px] mx-auto px-4 md:px-6">
    ...
    <div class="flex flex-col md:flex-row gap-12 lg:gap-16 mb-16">
      ...
      <!-- Follow Us -->
      <div class="flex-shrink-0">
        <h4 class="text-white font-bold text-lg mb-5">Follow Us</h4>
      ...
      <!-- Navigation -->
      <div class="flex-1" x-data="{ contactOpen: false, subOpen: false }">
        <h4 class="text-white font-bold text-lg mb-5">Navigation</h4>
        <div class="grid grid-cols-1 md:grid-cols-2 md:gap-x-16 gap-y-3">
          <a href="?view=home" class="text-sm text-neutral-400 hover:text-white transition-colors">Home</a>
          <a href="?view=about" class="text-sm text-neutral-400 hover:text-white transition-colors">About</a>
          <a href="?view=calendar" class="text-sm text-neutral-400 hover:text-white transition-colors">Calendar</a>
          <a href="?view=comedians" class="text-sm text-neutral-400 hover:text-white transition-colors">Comedians</a>
          <a href="?view=clips" class="text-sm text-neutral-400 hover:text-white transition-colors">Comedy Clips</a>
          <a href="?view=gift" class="text-sm text-neutral-400 hover:text-white transition-colors">Gift Certificates</a>
          <!-- Contact dropdown: desktop flyout, mobile always visible -->
          <!-- Desktop: flyout -->
          <div class="hidden md:block relative" @click.outside="contactOpen = false; subOpen = false">
            <button @click="contactOpen = !contactOpen" class="text-sm text-neutral-400 hover:text-white transition-colors flex items-center gap-1">
              Contact
              <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 transition-transform" :class="contactOpen ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
            </button>
